...
| 세부내역서 | 단가 | 79 (40/39) | 모델 추출 단계 오탐(raw에서도 fp 39, 룰 파생 아님) |
...
| 2 | `_fill` 오탐 억제: 환자정보/의료기관 영역 제약, 이름류 상호배제, 동률·모호 후보는 미채움, 소견서 진단일 보충 제한 | fp ~75, 회귀 18건 |
...
1번의 "항목명 정규화 시 세분 항목 보존" 가설은 구현 결과 사실이 아니었다. 정규화는 세분 항목명을 이미 살리고 있었고, 실제 원인은 `_receipt_table`이 파서 표에서 복원한 행을 모델 행과 짝짓지 않고 뒤에 끼워 넣어 행 위치가 밀리던 것이었다([kv-row-align](2026-09-22-kv-row-align.md)).

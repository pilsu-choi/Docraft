...
### 행 bbox 추정 — 폐기됨 (2026-09-22)

> **폐기됨 (2026-09-22)**: 아래 균등 분할 추정은 PP-OCRv5 줄 단위 OCR 좌표로 교체됐다. 자세한 내용은 [ocr-line-grounding](2026-09-22-ocr-line-grounding.md) 참고. 아래는 당시 구현의 기록이다.
...
## main 병합 시 조정 (2026-09-22)

main에 `parsers._html_table_rows()`가 생겨 새로 파싱한 table 블록은 `rows`를 갖게 됐다. 이 파서는 빈 `<tr>`을 건너뛰므로(검증 문서 기준 25행 중 24행) 그 `rows`로 행 인덱스를 세면 빈 행 뒤의 행 bbox가 한 칸씩 어긋난다. 그래서 `_block_rows()`는 블록 텍스트가 table HTML이면 `<tr>`을 직접 세는 쪽을 우선하고, HTML이 아닐 때만 `rows`를 쓴다. 두 함수는 목적이 달라(내용 행 vs 기하학적 행) 합치지 않았다.

## 남은 한계

- **같은 행 안의 컬럼 밀림은 못 잡는다.** `항목정보[0].일자`를 같은 행의 코드 값 `AA254`로 바꾸거나 한 행 안에서 필드를 통째로 한 칸씩 민 결과도 전부 `1.0`으로 남는다. 합의는 행 단위라서 행만 맞으면 통과한다. 이걸 잡으려면 표 헤더와 스키마 필드를 컬럼으로 매핑해야 하는데, 그건 별도 기능이다. 심각도 판단(2026-09-22): 사람이 결과를 검토·승인하는 현재 흐름에서는 마이너로 보고 보류했다. 이 밀림의 원인이던 strict 모드 키 정렬을 제거한 뒤 재현 테스트 4회에서 한 번도 나오지 않았기 때문이다. 다만 두 값이 모두 원문에 있어 `1.0`으로 통과하므로, 검토 없는 자동 승인이나 대량 처리로 가면 크리티컬에 가까워진다. 다른 양식 문서에서 밀림이 실제로 관측되면 컬럼 일관성 검사(배열 항목들끼리 같은 필드가 합의 행 안에서 같은 셀 인덱스에 있는지 비교해 최빈값과 다른 값을 0.5로 낮춤, 20~30줄, 추가 LLM 호출 없음)를 추가한다. 같은 행에 같은 값이 여러 셀에 있거나 모든 행이 똑같이 밀린 경우는 이 검사로도 못 잡는다. 금액 필드는 스키마에 산술 검증 규칙을 선언하는 구조가 생기면 그쪽이 더 확실하다.
- **행 bbox는 균등 분할 추정이다.** ~~`rowspan`을 무시하고 행 높이가 모두 같다고 가정하므로, 머리글이 2행으로 병합됐거나 행 높이가 들쭉날쭉한 표에서는 실제 행보다 위나 아래로 어긋난다. 검증 문서에서는 25행이 거의 균등해 눈에 띄는 오차가 없었지만 보장은 아니다. 셀 단위 정확한 좌표는 OCR 응답에 없으므로(위 참조) 파서가 표를 더 잘게 나누기 전에는 개선 여지가 없다.~~ **폐기·해결됨 (2026-09-22)**: 별도 PP-OCRv5 줄 단위 OCR 파이프라인의 실제 좌표로 교체해 이 한계를 없앴다. [ocr-line-grounding](2026-09-22-ocr-line-grounding.md) 참고.

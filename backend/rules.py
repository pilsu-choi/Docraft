...
- ``apply(doc_type, result, blocks)``: ``engine.extract`` 결과(정규 표현, doctypes 참고)에
  ...
  소견 문장에서 치료·검사 내역 행 만들기, 병명코드 분리, 인쇄되지 않은 급여 합계 비우기, 합계행 정리,
...
def _master_checks(fields):
    ...
    인쇄 표기가 달라 오탐이 절반을 넘어 알리지 않는다. ``master.similarity()`` 유사도 임계값으로
    다시 재봐도(76건 평가, sim<임계값이면 검토 필요) 정밀도 70%를 못 넘는다 — 진단명이 영문으로
    인쇄되면 마스터 한글 명칭과 유사도가 항상 0에 가까워 이미 맞는 값까지 걸리고, EDI는 인쇄
    표기(공백·구두점·어순)가 고시와 달라 낮은 유사도가 오류와 구분되지 않는다:

    | 계통 | 대상행 | 최고 정밀도 임계값 | 발동 | 참(true) | 거짓(false) | 정밀도 |
    |---|---:|---|---:|---:|---:|---:|
    | 병명(kcd) | 35 | <0.65 | 7 | 2 | 5 | 29% |
    | EDI | 120 | <0.55 | 43 | 9 | 34 | 21% |

    자세한 수치는 [wiki/2026-09-23-master-name-mismatch-flag.md](../wiki/2026-09-23-master-name-mismatch-flag.md).
    """

...
현재 범위는 진단서·소견서·수술확인서·입퇴원확인서·진료비영수증·세부내역서·약제비영수증 7종이다.
AO가 다른 이름으로 내는 유형(입원확인서·약제영수증)은 ``ALIASES``로 이 이름에 맞춘다.
...
_SPEC = {  # 진단서 계열 4종은 AO 스키마가 같다(값이 없는 표는 AO가 "[]" 스칼라로 내도 같은 키다).
    **dict.fromkeys(("진단서", "소견서", "수술확인서", "입퇴원확인서"), (_MEDICAL_FIELDS, _MEDICAL_TABLES)),
    "진료비영수증": ({
    ...
    "약제비영수증": ({
        "환자성명": ("text", "환자 성명. 한글 이름만, 나이·성별 표기 없이."),
        "발행일": ("date", "영수증 발행일. '발행일'·'발급일' 라벨의 값. 조제일자로 채우지 않는다. YYYYMMDD."),
        "병원명": ("text", "처방전을 발행한 의료기관 명칭. '처방전발행기관'·'처방기관'·'발행기관' 라벨의 값. 약국 상호가 아니다."),
        "사고발생일자": ("date", "사고(조제) 발생일. 조제일자와 같다. YYYYMMDD."),
        "조제일자": ("date", "조제일자. '조제일자'·'조제일' 라벨의 값. YYYYMMDD."),
        "진료비내역-총액": ("amount", "약제비 총액(①+②+③). '약제비총액'·'총액' 라벨의 값. 숫자만."),
        "진료비내역-급여본인부담": ("amount", "급여 본인부담금(①). '본인부담금' 라벨의 값. 숫자만."),
        "진료비내역-공단부담액": ("amount", "공단(보험자) 부담금(②). '보험자부담금'·'공단부담금' 라벨의 값. 숫자만."),
        "진료비내역-전액본인부담": ("amount", "전액본인부담금 칸이 따로 인쇄된 서식만 채운다. 숫자만."),
        "진료비내역-비급여및전액본인부담금": ("amount", "비급여 및 전액본인부담금(③). 칸이 비급여·전액본인부담으로 "
                                               "나뉘어 있으면 둘의 합. 숫자만."),
        "진료비내역-환자부담총액": ("amount", "환자부담총액(①+③). 숫자만."),
        "소득공제대상액-수납금액": ("amount", "총수납금액(카드·현금영수증·현금의 합계). 숫자만."),
        "약국정보(상호)": ("text", "약국 상호. '상호'·'약국명' 라벨의 값. 직인·대표자명 제외."),
        "약국정보(사업자등록번호)": ("amount", "약국 사업자등록번호. 숫자만."),

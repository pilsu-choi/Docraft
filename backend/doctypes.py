...
_RECEIPT_ITEM = {  # 진료비영수증 항목내역 열(항목 외에는 모두 금액)
    "항목": ("text", "표의 항목 칸에 인쇄된 진료 항목 구분명. 표준 목록에 없는 이름(의학료·혈액진단료 등)도 "
                     "인쇄된 대로 적는다. 여러 행을 묶는 분류 칸(기본항목·선택항목 등)은 항목명에 붙이지 않는다. "
                     "금액이 모두 빈칸·0인 행도 빠뜨리지 않는다. "
                     "항목명에서 한글·영문·숫자가 아닌 문자(가운뎃점·괄호·낫표·빗금·공백 등)는 모두 지운다"
                     "(보철·교정료→보철교정료, 검사/판독료→검사판독료). 아래 하위 항목 표기의 밑줄만 예외다. "
...
_SPEC = {
    ...
    "세부내역서": ({
        ...
        "급여_급여총액": ("amount", "급여(본인부담+공단부담+전액본인부담) 합계의 인쇄값. 급여 합계 칸이 없으면 "
                                 "null(직접 계산 금지). 비급여를 포함한 총액(진료비총액 개념)과는 다르다. 숫자만."),

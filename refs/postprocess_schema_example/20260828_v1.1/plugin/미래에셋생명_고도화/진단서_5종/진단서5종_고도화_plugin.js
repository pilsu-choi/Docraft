...
exports.plugin = async function (extractionResultData, schemaObj) {
  ...
  // Logger.log(extractionResultData.extraData)
  ...
  const diseaseCodeLength = values?.filter(v => v.img_extc_itnm === "병명코드").length

  values?.forEach((extrt) => {
    if(extrt?.img_extc_itnm ==="비고"){
      if(extrt?.extc_rst_cont01.includes("용도")){
        let idx = extrt?.extc_rst_cont01.indexOf("용도") +"용도".length
        const tmpText = extrt?.extc_rst_cont01.slice(idx)
        extrt.extc_rst_cont01=''
        extrt.extc_rst_cont01 = tmpText
      }
    }
    // '질병명'에서 질병분류번호 추출
    ...
    if (extrt?.extc_rst_cont01?.length > 0) {
      ...
      if (extrt?.img_extc_itnm == "발급일") {
        if(/^\d{14}$/.test(extrt?.extc_rst_cont01)){
          extrt.extc_rst_cont01 = extrt?.extc_rst_cont01.slice(0,8)
        }
      }
    ...
    const datePeriodPattern = new RegExp(
      `${datePattern}(?:부터|~|-)(~)?(입원|위환자는)?${datePattern}(?:까지)?`, 'g'
    );
    allText = allText.replace(/\s/g, "");
    ...
    if (extrt?.img_extc_itnm?.includes("병명코드")) {
      if (codeList?.length > diseaseCodeLength) {
        extrt.extc_rst_cont01 = codeList?.join('')
        codeList = []
      }
    }
    ...
    // console.log("!1111111111", extrt.extc_rst_cont01)
    ...
    if (extrt?.img_extc_itnm?.includes("통원일수") && extrt?.extc_rst_cont01?.length === 0) {
      ...
      const datePeriodPattern2 = new RegExp(
        `(${datePattern})(?:부터|~|-)(?:~)?(통원)(년월일)(?:까지)?(?:\\(?\\d+일간\\)?)?`
      );
      const datePeriodPattern3 = new RegExp(
        `(총)(?:\\d+일간)`
      );
      const visitdatesPattern = /[0-9]{1,2}(일간)/g
      allText = allText.replace(/\s/g, "");
      const periodMatch = allText?.match(datePeriodPattern) || allText?.match(datePeriodPattern2) || allText?.match(datePeriodPattern3) || "";
      if (allText?.match(datePeriodPattern2)) {
        visitDay = allText?.match(datePeriodPattern2)[0]?.match(dateRegex)[0]?.replace(/[^0-9]/g, "") || ""
      }
    ...
    if(extrt?.img_extc_itnm?.includes("병원연락처")&&extrt?.extc_rst_cont01?.length==0){
      tel_tmp = allText.match(/tel\.?\s*(.*?)\s*fax/i)?.[1] ?? "";
      // tel_tmp = allText.replace(/.*tel\.?\s*(.*?)\s*fax.*/i,"$1") || ""
    }
  ...
  values?.forEach(values => {
    if (values?.img_extc_itnm == "통원일" || values?.img_extc_itnm.includes("일자")) {
      ...
      values.extc_rst_cont01 = values.extc_rst_cont01?.replace(/[월]/g, "")
      if (values.extc_rst_cont01 == "0") {
        values.extc_rst_cont01 = ""
      }
      if (values?.img_extc_itnm == "통원일" && values?.extc_rst_cont01 == '' && visitDay) {
        values.extc_rst_cont01 = visitDay
      }
    }
  ...
  let tmpYear = ''
  values?.forEach((obj) => {
    if (obj?.img_extc_itnm == "진단일" && obj.extc_rst_cont01?.length == 8) {
  ...
  values?.forEach((obj) => {
    ...
    if (obj?.img_extc_itnm == "차트번호" || obj?.img_extc_itnm == "환자 등록번호") {
      ...
      obj.extc_rst_cont01 = obj.extc_rst_cont01?.replace(":", "")
    }
    ...
    if (obj.img_extc_itnm == "병명코드") {
      ...
      // console.log("########",obj.extc_rst_cont01)
    }
    ...
    if (obj.img_extc_itnm == "병원연락처") {
      ...
      if (obj?.extc_rst_cont01?.length > 12 &&(obj?.extc_rst_cont01?.match(phonePattern))) {
        obj.extc_rst_cont01 = obj?.extc_rst_cont01?.match(phonePattern)[0] || obj.extc_rst_cont01;
      }
      if(obj.extc_rst_cont01?.length==0){
        obj.extc_rst_cont01 = tel_tmp
      }
    }
  ...
  // pluginUtil.diagExcel('excel', values, extractionResultData, keyObj);
...
function splitDiagCode(values) {
  ...
  values.forEach((item) => {
    if (item?.img_extc_itnm === "병명코드" && item?.extc_rst_cont01) {
      ...
      // console.log("3333333333",cleaned)

      // 2) "영문 1자리 + 숫자 3~5자리" 단위로 추출 (예: D2420, N608)
      const codeMatches = cleaned.match(/[A-Za-z][0-9]{2,3}(?:\.[0-9]{1,3}|[0-9]{1,3})?/g);
    ...
    // console.log("%%%%%%%%%%%%%%%",item?.extc_rst_cont01)
...
function splitMultiValueItem(values, diagPeriod, diagIN, diagOUT) {
  ...
  const visitIdx = dp.indexOf("통원");
  let inText = ''
  let visitText = ''
  if (inIdx !== -1) {
    if (visitIdx !== -1 && visitIdx > inIdx) {
      inText = dp.slice(inIdx, visitIdx); // "입원" 부터 "통원"전까지
      visitText = dp.slice(visitIdx); // "입원" 부터 "통원"전까지
      // 1) YYYY... 먼저
      inoutDates = inText.match(fullDateRegex) ?? [];
    }
    else {
      inText = dp.slice(inIdx); // "입원" 이후만
      // 1) YYYY... 먼저
      inoutDates = inText.match(fullDateRegex) ?? [];
    }
    // 2) 없으면 YYMMDD fallback
  ...
  if (inoutDates?.length == 0 && diagPeriod && visitIdx < 0) {
  ...
  if(inoutPairs?.length==0 && diagPeriod?.length>0){
    if (diagPeriod.match(fullDateRegex)) {
      for (let i = 0; i < diagPeriod.match(fullDateRegex)?.length; i++) {
        inoutPairs.push([diagPeriod?.match(fullDateRegex)[i], diagPeriod?.match(fullDateRegex)[i + 1]])
        i = i + 1;
  ...
  values.forEach((item) => {
    ...
    // =========================================
    if (item?.img_extc_itnm == "통원일" && (item?.extc_rst_cont01?.length ==0) ) {
        item.extc_rst_cont01 = visitText
    }
    if (item?.img_extc_itnm == "통원일" && (item?.extc_rst_cont01?.length ?? 0) > 0) {
      const raw = item.extc_rst_cont01 ?? visitText ?? "";
      ...
      if (raw.match(/([0-9]{16})/g)) {
        matches = raw.match(/([0-9]{8})/g) ??[];
      } else {
        matches = raw.match(datePattern) ?? [];
      }
    ...
    if (
      ...
      for (const p of parts) {
        ...
        const addPattern = /^[A-Za-z]\d{2,3}(?:\.\d+)?$/;
        if (addPattern.test(diseaseStr)) {
          continue;
        }
    ...
    if (
      ...
      let parts = item.extc_rst_cont01
        .split("$$$")
        .map(v => v.trim())
        .filter(Boolean);
      if (parts.length > 0) {
        if (parts[0].length < 10) {
          ...
          for (let i = 0; i < parts.length - 2; i++) {
            if (
              ...
              /월$/.test(parts[i + 1]) &&
              /일$/.test(parts[i + 2])) {
              parts_tmp.push(parts[i] + parts[i + 1] + parts[i + 2])
              i += 2;
            }
          }
          parts = []

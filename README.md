...
룰 검사(`rules.check`)는 표 금액 셀의 겹침·없는 열·합계 베끼기·행 누락과 과다·합계식 불일치(`sum_mismatch`)·이웃 열 밀림(`column_shift`)·행 산술(`row_arith`)·날짜 선후(`bad_date`)·주민번호 불일치(`id_mismatch`)·인쇄되지 않은 합계(`ungrounded`)·저품질 문서(`low_quality`)·마스터에 없는 병명코드를 찾습니다. 그중 확실한 것(없는 열 값 옮기기, 금액 0인 누락 행, 옮겨갈 칸이 빈 열 밀림, 통째로 맞바뀐 열, 인쇄되지 않은 세부내역서 급여 합계)만 `rules.correct`가 교정하고 나머지는 Judge 힌트로 넘깁니다. 유형별 엣지케이스와 효과는 [룰 엣지케이스 기록](wiki/2026-09-23-rules-edge-cases.md)에 있습니다.
